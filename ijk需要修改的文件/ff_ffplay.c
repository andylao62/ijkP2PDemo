...
static int stream_has_enough_packets(AVStream *st, int stream_id, PacketQueue *queue, int min_frames) {
    ...
    //st->disposition & AV_DISPOSITION_ATTACHED_PIC 代表只是mp3文件的封面数据，虽然也是video ，但不作为video使用
    return stream_id < 0 ||
           queue->abort_request ||
...
//        err = avformat_find_stream_info(ic, 0);
    ...
    printf("[wl]start loop readframe!!!!\n");
    //loopRead
    for (;;) {
//        SDL_Delay(20);
        if (is->abort_request)
...
//      of the seek_pos/seek_rel variables
        ...
        if (ret < 0) {
            printf("[cloudVideoMux]av_read_frame: %s\n", av_err2str(ret));
            int pb_eof = 0;
            int pb_error = 0;
            ...
            is->eof = 0;
//            printf("av_read_frame right\n");
        }

            //不是连续的部分，就添加flush_pkt作为标记
